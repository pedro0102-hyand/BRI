...
TOP_K_RESULTS = 10
OUTPUT_PATH = REPORTS_DIR / "es_combo7_results.json"

# busca na base de dados do Elasticsearch usando uma subconsulta
def search_subquery(es, tokens: list[str], limit: int = 10):
    if not tokens:
        return []
    query = {
        "query": {"match": {"content": " ".join(tokens)}},
        "size": limit,
    }

    # Executa a consulta no Elasticsearch
    response = es.search(index=INDEX_NAME, body=query)
    return [(hit["_source"]["doc_id"], hit["_score"]) for hit in response["hits"]["hits"]]

# busca documentos suspeitos no Elasticsearch usando subconsultas extraídas do texto
def search_suspicious_document(es, text: str) -> list[tuple[str, float]]:

    subqueries = extract_subqueries(text) # extrai subconsultas do texto
    aggregated = defaultdict(float) # cria um dicionário para agregar os scores dos documentos
...
if __name__ == "__main__":
    ...
    print(f"Buscando {len(queries)} documentos suspeitos (Elasticsearch, combinação 7)...")
    results = {}
